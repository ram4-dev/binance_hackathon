...
describe("runRealtimeDialogue empty-response retry", () => {
  it("retries response.create when the API returns an empty response.done", async () => {
    const { runRealtimeDialogue } = await import(
      "../../evals/voice/realtime/session.js"
    );
    const script: Array<(ws: FakeWs) => void> = [
      ...
      (ws) => {
        ...
        emit(ws, "response.output_audio_transcript.delta", {
          delta: { transcript: "Tu saldo es 42.5" },
        });
        emit(ws, "response.done", {
    ...
    const result = await runRealtimeDialogue(
      ...
      noopBinding,
      {
        wsFactory: () => ws as unknown as InstanceType<typeof WebSocket>,
        setupTimeoutMs: 1000,
